%%   CLEANED UP GLACIER MODEL FOR PINE CREEK
%
...
%  set up distance array
    ...
    xmax  = 19466;    % changes based on what data you'd like to import
       dx = xmax/step;
        x = dx/2:dx:xmax-(dx/2);
    xedge = 0:dx:xmax;
 
% pine creek valley width as a function of distance downvalley (approximated)
 
    W_min2 = 800; % meters
    m = 3;
    phi2 = 3; % importance of tributary widening
    x_star2 = 2000; % how quickly does it shrink ?
    x_max2 = 19466;
    dx = x_max2/step;
    x2 = 0:dx:x_max2-1;
    shift2 = 1000;
    geom2 = (1 + phi2.*(((x2+shift2)/x_star2).^m).*exp(-((x2+shift2)/x_star2)));
    ...
    % LOAD IN THE PINE CREEK TOPOGRAPHY
    load PC_new_profile.txt 
        N_data = 660;
    % with SMOOTHING FUNCTION
    zb = transpose(smooth((PC_new_profile(1:N_data/step:end)))); 
...
if rem(t(i),tplot)==0
    ...
    ylabel('Elevation [m]','fontname','arial','fontsize',font) 
        ...
        ELA0_text2 =strcat('ELA center (',ELA0_text,' m)');
           legend('temperate valley glacier','smoothed bed topography','ELA range')
    ...
    %   now define the analytic solution:
        Qanal = (cumsum(b)*dx);      % steady state ice discharge w/out width
    ...
    axis([0 xmax/1000 0 20])
        legend('total ice discharge','integrated b(z)')
